...
async def bot_echo(call:types.CallbackQuery):
    ...
    await call.message.answer("14 це:",reply_markup=dynamik_kb2_3(answers[1]))


@dp.callback_query_handler(text="0",state=test2_3.p_1)
async def bot_echo(call:types.CallbackQuery):
    await test2_3.p_2.set()
    await call.message.answer("скільки років бравл старсу?",reply_markup=dynamik_kb2_3(answers[2]))

@dp.callback_query_handler(text="2",state=test2_3.p_2)
async def bot_echo(call:types.CallbackQuery):
    await test2_3.P_3.set()
    await call.message.answer("кавун це:",reply_markup=dynamik_kb2_3(answers[3]))
